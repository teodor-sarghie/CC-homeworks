...
class GoogleDrive:
    def connect(self):
        ...
        if os.path.exists("Drives/token.json"):
            self.creds = Credentials.from_authorized_user_file("Drives/token.json", SCOPES)
    ...
    def upload_file_directly(self, file_obj, file_name, mime_type='application/octet-stream'):
        service = self.get_service()
